...
IF EXISTS(SELECT * FROM sys.tables WHERE name = 'bigTransactionHistory')
DROP TABLE dbo.bigTransactionHistory
GO

IF EXISTS(SELECT * FROM sys.tables WHERE name = 'bigTransactionHistory2')
DROP TABLE dbo.bigTransactionHistory2
GO

IF EXISTS(SELECT * FROM sys.tables WHERE name = 'bigProduct')
DROP TABLE dbo.bigProduct
GO

IF EXISTS(SELECT * FROM sys.tables WHERE name = 'bigProduct2')
DROP TABLE dbo.bigProduct2
GO
...
WHERE
	a.type = 'p'
	AND a.number BETWEEN 11 AND 30
GO


ALTER TABLE bigProduct2
ALTER COLUMN ProductId INT NOT NULL	
GO

ALTER TABLE bigProduct2
ADD CONSTRAINT pk_bigProduct2 PRIMARY KEY (ProductId)
GO
...
ALTER TABLE bigTransactionHistory2
ALTER COLUMN TransactionID INT NOT NULL
GO


ALTER TABLE bigTransactionHistory2
ADD CONSTRAINT pk_bigTransactionHistory2 PRIMARY KEY (TransactionID)
GO


CREATE NONCLUSTERED INDEX IX_ProductId_TransactionDate2
ON bigTransactionHistory2
(
	ProductId,
	TransactionDate
)
